package tr.edu.medipol.ogrenciler.batinarslan.odev6;
...
public class Odev6Islemler extends IslemlerGeneric<Ogrenci,String>{
	...
	public boolean kayitSil(String kayitNo) {
		for(int i=0; i<kayitlar.size(); i++){
			...
			if(ogrenci.getNo().equals(kayitNo)){
				System.out.println("numaras�= " + ogrenci.getNo() + " Ad� ve Soyad�= "+ogrenci.getAdSoyad() + "��renci Tipi= " + ogrenci.ogrenciTipi());
				kayitlar.remove(i);
			}
	...
	public SortedSet kayitSortedSet() {
		...
		SortedSet<Ogrenci> kayitset = new TreeSet<Ogrenci>();
	    kayitset.forEach(System.out::println);
		return null;
	}
