...
public class VeritabaniYonetimi {
	...
	public static boolean tablolariOlustur() {
		...
		try {
			...
			veritabaniStatement.executeUpdate("CREATE TABLE " + "urunler (" + "urunid int PRIMARY KEY, "
					+ "urunadi varchar(15), " + "urunkategori varchar(10), " + "fiyat int " + ")");
			// "uyeler" tablosu: kullanici_adi, kullanici_tipi, yas, cinsiyet, sifre
			veritabaniStatement.executeUpdate("CREATE TABLE " + "uyeler (" + "kullanici_adi varchar(40), "
					+ "kullanici_tipi varchar(15), " + "yas int, " + "cinsiyet varchar(10), " // E-K
					+ "sifre varchar(25)" + ")");
	...
	public static User kullaniciSorgula(String kullaniciAdi) {
		...
		try {
			ResultSet sonuclar = veritabaniStatement
					.executeQuery("SELECT * FROM uyeler WHERE kullanici_adi = '" + kullaniciAdi + "'");
			while (sonuclar.next()) {
	...
	public static void kullanicilariOlustur() {
		...
		try {
			// "kullanicilar" tablosu: kullanici_adi, kullanici_tipi, yas, cinsiyet, sifre
			veritabaniStatement
					.executeUpdate("INSERT INTO uyeler VALUES ( " + "'guest', 'NORMAL', 22, 'E', 'abcd'" + ")");
			veritabaniStatement
					.executeUpdate("INSERT INTO uyeler VALUES ( " + "'admin', 'ADMIN', 22, 'K', 'xyz'" + ")");
	...
	public static void kayitol(String kullaniciadi, String yas, String cinsiyet, String Sifre) {
		try {
			...
			try {
				veritabaniStatement.executeUpdate("INSERT INTO uyeler VALUES(" + "'" + kullaniciadi + "','NORMAL',"
						+ yass + ",'" + cinsiyet + "','" + Sifre + "')");
			} catch (Exception e) {
	...
	public static void urunleriolustur() {
		...
		try {
			// "kullanicilar" tablosu: kullanici_adi, kullanici_tipi, yas, cinsiyet, sifre
			veritabaniStatement.executeUpdate("INSERT INTO urunler VALUES ( " + "1, 'Ekmek', 'Manav', 5" + ")");
			veritabaniStatement.executeUpdate("INSERT INTO urunler VALUES ( " + "2, 'Soðan', 'Manav', 7" + ")");
	...
	public static Urun urunsorgula(String urunadi) {
		...
		try {
			ResultSet sonuclar = veritabaniStatement
					.executeQuery("SELECT * FROM urunler WHERE urunadi = '" + urunadi + "'");
			while (sonuclar.next()) {
	...
	public static List<Urun> urunlerilistele() {
		...
		try {
			ResultSet sonuclar = veritabaniStatement
					.executeQuery("SELECT * FROM urunler");
			while (sonuclar.next()) {
				String urunadi = sonuclar.getString("urunadi");
				urunler.add( new Urun(urunadi) );
			}
		} catch (Exception e) {
			System.out.println("ürün sorgu hata" + e.getMessage());
		}
